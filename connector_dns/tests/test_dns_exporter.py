...
class TestDNSExporter(SetUpDNSBase):
    ...
    def test_export_dependency_unwrapped(self):
        ...
        with self.mock_base_exporter(exporter):
            ...
            with self.assertRaises(self.EndTestException):
                exporter._export_dependency(expect, self.model)
            to_backend.assert_called_once_with(expect, wrap=False)
    ...
    def test_map_data_call(self):
        ...
        with self.mock_base_exporter(exporter):
            exporter._map_data()
            exporter.mapper.map_record.assert_called_once_with(exporter.binding_record)
    ...
    def test_create_data_call(self):
        ...
        fields = expect.keys()
        self._new_exporter(self.dns_id)._create_data(map_record, fields, **expect)
    ...
    def test_update_data_call(self):
        ...
        expect = {"test": 123, "test2": 456}
        fields = expect.keys()
        self._new_exporter(self.dns_id)._update_data(map_record, fields, **expect)
    ...
    def test_run_update_no_record(self):
        ...
        with self.mock_base_exporter(
            ...
            with self.assertRaises(self.EndTestException):
                exporter._run()
            exporter._update.assert_called_once_with(exporter._update_data())
    ...
    def test_run_create_no_record(self):
        ...
        with self.mock_base_exporter(
            ...
            with self.assertRaises(self.EndTestException):
                exporter._run()
            exporter._create.assert_called_once_with(exporter._create_data())
