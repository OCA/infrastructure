...
class TestDNSImporter(SetUpDNSBase):
    def setUp(self):
        ...
        self.mock_env = self.get_dns_helper(self.model)
    ...
    def test_import_dependency_always(self):
        ...
        with self.mock_base_importer(importer):
            ...
            with self.assertRaises(self.EndTestException):
                importer._import_dependency(True, self.model, always=True)
    ...
    def test_import_dependency_no_odoo_binder(self):
        ...
        with self.mock_base_importer(importer):
            ...
            importer.binder_for().to_openerp.assert_called_once_with(self.dns_id)
    ...
    def test_import_dependency_runs_import(self):
        ...
        importer = self._new_importer()
        with self.mock_base_importer(importer):
            importer._import_dependency(self.dns_id, self.model, always=True)
            importer.unit_for().run.assert_called_once_with(self.dns_id)
    ...
    def test_map_data_call(self):
        ...
        with self.mock_base_importer(importer, ["_mapper"]):
            importer._map_data()
            importer.mapper.map_record.assert_called_once_with(self.dns_record)
    ...
    def test_create_data_call(self):
        ...
        self._new_importer(self.dns_id)._create_data(map_record, **expect)
        map_record.values.assert_called_once_with(for_create=True, **expect)
    ...
    def test_create_returns_binding(self):
        ...
        with self.mock_base_importer(importer, ["connector_env"]):
            ...
            self.assertEqual(importer.connector_env.model.with_context().create(), res)

    def test_update_data_call(self):
        """It should inject proper vals into map record"""
        map_record = mock.MagicMock()
        expect = {"test": 123, "test2": 456}
        self._new_importer(self.dns_id)._update_data(map_record, **expect)
    ...
    def test_run_does_force(self):
        ...
        importer = self._new_importer()
        with self.mock_base_importer(importer, ["_before_import", "_is_current"]):
            importer._must_skip.return_value = False
