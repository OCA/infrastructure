...
class DNSBackend(models.Model):
    ...
    login = fields.Char(string="Login", help="Provider's login.", required=True)
    password = fields.Char(
        string="Password", help="Provider's password.", required=True
    )
    uri = fields.Char(
    ...
    def _import_from_date(self, model, from_date_field, chg_date_field=None):
        ...
        for backend in self:
            ...
            if from_date:
                filters[chg_date_field][">="] = fields.Datetime.from_string(from_date)
    ...
    def import_dns_zones(self):
        self._import_from_date("dns.zone.bind", "import_zones_from_date")

    @api.multi
    def import_dns_records(self):
        self._import_from_date("dns.record.bind", "import_records_from_date")

    @api.model
    def _get_session(self):
        """It returns a ConnectorSession for the environment"""
        return ConnectorSession(self.env.cr, self.env.uid, context=self.env.context)
